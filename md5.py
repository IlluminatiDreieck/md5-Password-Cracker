...
if __name__=='__main__':
    ...
    tried = []
    tries = 0
    characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!\"§$%&/()=ß\\´`#+*';:_-.,<>|"
    for n in range(0, len(characters)):
        message = characters[n]
    ...
    while finished == "false":
        beforeend = lastend
        for n in range(laststart, lastend):
            for m in range(0, len(characters)):
                message = tried[n] + characters[m]
                hashedmessage = hash(message)
                tries += 1
                if finished == "false":
                    print(hashedmessage, " <-- ", tries, ": ", message)
